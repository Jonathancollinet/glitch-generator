...
export default function getGlitchColorFrom(shadow: string) {
    ...
    if (colorParse) {
        ...
        const nameParse = shadow.match(/^[a-z]+/)?.[0];

        if (nameParse) {
            if (nameParse === "transparent") {
                fieldColor.alphaPercent = 0;
            } else {
                const hex = colourNameToHex(nameParse as string);
...
function colourNameToHex(colourName: string) {
    ...
    return colours[colourName as keyof typeof colours];
}
