...
<template>
    <div v-if="field">
        ...
        <div class="text-xs mb-1" v-if="textShadow?.enabled">
            ...
            <div class="flex pl-1">{{ formatTextShadow }}
                <div class="ml-1 h-3 w-3"
                    :style="{ background: hexToRGB(textShadow.color.hex, textShadow.color.alphaPercent) }" />
            </div>
        </div>
        <div class="text-xs" v-if="boxShadow?.enabled">
            ...
            <div class="flex pl-1">{{ formatBoxShadow }}
                <div class="ml-1 h-3 w-3"
                    :style="{ background: hexToRGB(boxShadow.color.hex, boxShadow.color.alphaPercent) }" />
            </div>
        </div>
    </div>
</template>